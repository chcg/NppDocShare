...
int Socket::recieveData(char * buffer, int buffersize) {
	...
	if (result == SOCKET_ERROR) {
		...
		//Proper shutdown, still return -1
		return -1;
	}
	return result;
}

int Socket::recieveDataComplete(char * buffer, int buffersize) {
	int total = 0, res;
	while(total < buffersize) {
		res = recieveData(buffer+total, buffersize-total);
		if (res == -1)
			return -1;
		total += res;
	}
